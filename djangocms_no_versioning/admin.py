...
from .constants import INDICATOR_DESCRIPTIONS, PUBLISHED, VERSION_STATES
    ...
    def get_queryset(self, request):
        ...
        for field in versionable.extra_grouping_fields:
            if hasattr(self.model_admin, f"get_{field}_from_request"):
                grouping_filters[field] = getattr(
                    self.model_admin, f"get_{field}_from_request"
                )(request)
            elif field == "language":
...
class StateIndicatorMixin(metaclass=MediaDefiningClass):
        ...
        def indicator(obj):
            ...
            return render_to_string(
                "admin/djangocms_versioning/indicator.html",
                {
                    ...
                    "menu": json.dumps(
                        render_to_string(
                            "admin/cms/page/tree/indicator_menu.html",
                            {"indicator_menu_items": menu},
                        )
                    )
                    if menu
                    else None,
    ...
    def get_list_display(self, request):
        ...
        if versionables.exists_for_content(
            self.model
        ) or versionables.exists_for_grouper(self.model):
            return tuple(
                self.get_indicator_column(request)
                if item == "state_indicator"
                else item
...
class ExtendedListDisplayMixin:
        ...
        for field in modifier_dict:
            ...
            try:
                ...
                raise ImproperlyConfigured(
                    "The target field does not exist in this context"
                ) from None
    ...
    def get_list_display(self, request):
        ...
        if isinstance(self, GrouperModelAdmin):
            modifier_dict = extension.add_to_field_extension.get(
                self.content_model, None
            )
        else:
            modifier_dict = extension.add_to_field_extension.get(self.model, None)
        if modifier_dict:
            list_display = self.extend_list_display(
                request, modifier_dict, list_display
            )
        return list_display
...
class ExtendedGrouperVersionAdminMixin(ExtendedListDisplayMixin):
    ...
    def get_queryset(self, request: HttpRequest) -> models.QuerySet:
        ...
        versions = Version.objects.filter(
            object_id=OuterRef("pk"), content_type__in=grouper_content_type
        )
        contents = self.content_model.admin_manager.latest_content(
            **{self.grouper_field_name: OuterRef("pk"), **self.current_content_filters}
        ).annotate(
            content_created_by=Subquery(
                versions.values(f"created_by__{conf.USERNAME_FIELD}")[:1]
            ),
        ...
        qs = qs.annotate(
            ...
            content_created_by_sort=Lower(
                Subquery(contents.values("content_created_by")[:1])
            ),
            content_state=Subquery(contents.values("content_state")),
            # cast is necessary for mysql
            content_modified=Cast(
                Subquery(contents.values("content_modified")[:1]),
...
class ExtendedVersionAdminMixin(
    ...
    def get_list_display(self, request):
        ...
        list_display += self.versioning_list_display + (
            self.get_admin_list_actions(request),
        )
        return list_display


class ExtendedIndicatorVersionAdminMixin(
    StateIndicatorMixin, ExtendedVersionAdminMixin
):
    versioning_list_display = (
        "get_author",
        "get_modified_date",
...
class VersionAdmin(
    ChangeListActionsMixin, admin.ModelAdmin, metaclass=MediaDefiningClass
):
    ...
    def content_link(self, obj):
        ...
        warnings.warn(
            "VersionAdmin.content_link is deprecated.", DeprecationWarning, stacklevel=2
        )
        content = obj.content
    ...
    def _get_unlock_link(self, obj, request):
        ...
        unlock_url = reverse(
            f"admin:{obj._meta.app_label}_{self.model._meta.model_name}_unlock",
            args=(obj.pk,),
        )
        return self.admin_action_button(
            unlock_url,
    ...
    def unlock_view(self, request, object_id):
        ...
        if not request.user.has_perm("djangocms_versioning.delete_versionlock"):
            return HttpResponseForbidden(
                force_str(_("You do not have permission to remove the version lock"))
            )
